...
interface PostWithComment {
  postId: number,
  title: string,
  totalComments: number;
}

export async function mapPostWithCommentCount():Promise<PostWithComment[]> {
  try{
    const response1 = await axios.get<Post[]>('https://jsonplaceholder.typicode.com/posts')
    const response2 = await axios.get<Comment[]>('https://jsonplaceholder.typicode.com/comments')
    const comments = response2.data
    const posts = response1.data
    if(!posts.length) return[]
    const mappost = posts.map((p)=>{
      const countcomment = comments.filter((f)=> f.postId==p.id)
      return {
        postId:p.id,
        title:p.title,
        totalComments:countcomment.length }})
    return mappost
  }
  catch(error){
    return[]
